...
for(const name of ['index.html','styles.css','typography.css','course.css','snap.css','src','data','icon.svg','icon-192.png','icon-512.png','manifest.webmanifest','privacy.html','CONTENT-LICENSE.md'])await cp(path.join(root,name),path.join(target,name),{recursive:true});
