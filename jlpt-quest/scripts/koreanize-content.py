...
META_PAREN=re.compile(r'\((?:\\d+|abbr|arch|col|comp|conj|exp|fam|hon|id|int|n|nav\\.?|uk|adj|adv|vs|vi|vt|de:[^)]*|nl:[^)]*|pt:[^)]*)\)',re.I)
LATIN_TOKEN=re.compile(r'(?<![가-힣])[A-Za-z][A-Za-z0-9+._^/-]*(?![가-힣])')

def clean_source(text:str)->str:
    text=META_PAREN.sub(' ',text)
...
def clean_translation(text:str)->str:
    ...
    if re.search(r'원래\s*제목|original\s*title',text,re.I):
        return ''
    # Keep Korean semantics, remove residual source-language abbreviations/roman tokens.
