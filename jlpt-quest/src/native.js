...
export async function judgeInk(lines,expected){if(!isNative())return judgeShape(lines,expected);try{const result=await callNative('recognize',{strokes:lines,width:1000,height:1000});return judgeRecognized(result.candidates,expected,lines);}catch(e){return {status:'unavailable',correct:false,method:'mlkit',reason:e.message};}}
