...
function curriculum(){
 ...
 <section class="panel pack-banner"><div><span class="label">${l}</span><h2>${TITLES[l]}</h2><p>${p.installed.toLocaleString()}단어 · ${list.length}개 챕터</p><p class="fine">${p.complete?`전체 공개팩 설치됨 · 한국어 뜻`:`기본 한국어 단어 · 전체팩 원본 ${p.available.toLocaleString()}개`}</p></div>${btn('packs',icon('book')+' 데이터 정보','soft')}</section>
...
function settings(){openModal(`${modalHead('내 속도에 맞춰요')}<div class="setting-row"><div><b>히라가나 표시</b><p>단어의 읽기를 켜거나 끄기</p></div><input type="checkbox" data-setting="furigana" aria-label="히라가나 표시" ${state.settings.furigana?'checked':''}></div><div class="setting-row"><div><b>화면 애니메이션</b><p>기기의 동작 줄이기도 반영해요</p></div><input type="checkbox" data-setting="motion" aria-label="화면 애니메이션" ${state.settings.motion?'checked':''}></div><div class="setting-row"><div><b>일본어 재생 속도</b></div><select data-setting="rate" aria-label="재생 속도">${[.7,.85,1].map(n=>`<option value="${n}" ${n===state.settings.rate?'selected':''}>${n}×</option>`).join('')}</select></div><div class="setting-row"><div><b>펜 굵기</b></div><select data-setting="penWidth" aria-label="펜 굵기">${[3,4,6].map(n=>`<option value="${n}" ${n===state.settings.penWidth?'selected':''}>${n}px</option>`).join('')}</select></div><div class="setting-row"><div><b>하루 목표</b></div><select data-setting="goal" aria-label="하루 목표">${[5,10,20,30].map(n=>`<option value="${n}" ${n===state.settings.goal?'selected':''}>${n}문제</option>`).join('')}</select></div><div class="modal-actions">${btn('export','기록 백업','soft')}${btn('import','백업 가져오기','soft')}</div><input id="backup-file" type="file" accept="application/json,.json" class="sr-only"><div class="spaced">${btn('packs',icon('book')+' 내장 단어 데이터','soft wide')}</div>${isNative()?`<p class="info-note">손글씨 판정과 모든 획 데이터가 APK에 내장돼 있어요. 첫 실행부터 인터넷 없이, 한 획씩 확인하고 제자리로 보정합니다. 외부 모델이나 별도 설치가 필요 없어요.</p>`:`<p class="info-note">앱과 같은 획순·위치·모양 판정을 사용합니다. 이 기능은 정해진 글자를 따라 쓰는 학습 기능이며 자유 필기 OCR은 아닙니다.</p>`}<p class="fine spaced">오프라인 발음: HTS Voice “NIT ATR503 M001” (CC BY 3.0). Nagoya Institute of Technology / Tokyo Institute of Technology. 자세한 고지는 아래 저작권 페이지에서 확인할 수 있어요.</p><div class="legal-links"><a class="quiet-link" href="./privacy.html">개인정보처리방침</a><a class="quiet-link" href="./terms.html">이용약관</a><a class="quiet-link" href="./licenses.html">오픈소스·저작권</a></div><div class="spaced">${btn('reset','학습 기록 초기화','text danger')}</div>`);}
function packsModal(){openModal(`${modalHead('N5부터 N1까지 · 내장 데이터')}<p class="fine">모든 급수의 단어·뜻·획·발음이 앱에 들어 있어 별도 다운로드가 필요하지 않습니다. 화면에 표시되는 뜻은 한국어만 사용합니다.</p><div class="pack-levels">${LEVELS.map(l=>{const p=packInfo(l);return `<div class="pack-level"><div><b>${l}</b><p>${p.installed.toLocaleString()}단어 내장</p><small>한국어 뜻 ${p.korean.toLocaleString()} · 영어 표시 0</small></div><span class="label">오프라인</span></div>`;}).join('')}</div><p class="fine spaced">자동 번역으로 만든 한국어 뜻은 정식 출시 전 교육 검수가 필요합니다. 영어 원문은 내부 출처 데이터로만 보존하고 학습 화면에는 표시하지 않습니다.</p>`);}
