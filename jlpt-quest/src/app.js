...
function packsModal(){if(isNative()){openModal(`${modalHead('앱에 모두 담겨 있어요')}<p>N5~N1 단어팩과 획 데이터가 APK 안에 들어 있어요. 다운로드나 손글씨 모델 준비 없이 사용할 수 있습니다.</p><div class="pack-levels">${LEVELS.map(l=>`<div class="memory-row"><b>${l}</b><span>${packInfo(l).installed.toLocaleString()}단어 내장</span></div>`).join('')}</div><p class="fine">일본어 발음 파일도 APK에 모두 포함되어 첫 실행부터 오프라인으로 재생됩니다. 대부분의 전체팩 뜻은 영어 원문이며 한국어 검수는 진행 전입니다.</p>`);return;}openModal(`${modalHead('N5부터 N1까지')}<p class="fine">OpenJLPT 고정 버전에서 내려받습니다. 기본 단어는 한국어, 전체팩 대부분은 영어 뜻이며 한국어 검수가 남아 있어요. 다운로드 시 GitHub에 접속 정보가 전달됩니다. 학습 기록·필기는 전송하지 않습니다.</p><div class="pack-levels">${LEVELS.map(l=>{const p=packInfo(l);return `<div class="pack-level"><div><b>${l}</b><p>${p.installed.toLocaleString()}개 설치 / 원본 ${p.available.toLocaleString()}항목</p><small>${p.complete?`전체팩 · 영어 뜻 ${p.english.toLocaleString()}개`:'한국어 시작 단어 60개'}</small></div>${btn('install-level',p.complete?'다시 받기':'전체팩 설치','soft',`data-level="${l}" ${downloading?'disabled':''}`)}</div>`;}).join('')}</div>${btn('install-all','5개 급수 전체팩 받기','primary wide',downloading?'disabled':'')}<p class="fine spaced">공식 전수 목록이 아닌 공개 자료입니다. 중복 표기는 병합하며 설치 후 실제 단어 수를 표시합니다. 다른 뜻의 예문이 섞인 항목은 일괄 제외합니다.</p><p id="pack-status" role="status"></p>`);}
