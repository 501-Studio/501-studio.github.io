...
function curriculum(){
 ...
 <section class="panel pack-banner"><div><span class="label">${l}</span><h2>${TITLES[l]}</h2><p>${p.installed.toLocaleString()}단어 · ${list.length}개 챕터</p><p class="fine">${p.complete?`전체 공개팩 설치됨 · 한국어 뜻`:`기본 한국어 단어 · 전체팩 원본 ${p.available.toLocaleString()}개`}</p></div>${btn('packs',icon('download')+' 단어팩','soft')}</section>
...
function questionPage(){const s=state.session;if(!s)return `<div class="focus-shell">${heading('시작한 수업이 없어요.')}${btn('home','홈으로','primary')}</div>`;if(s.finished)return resultPage();const t=current(s),w=W(t.wordId);if(!w)return '<div class="loading">수업 단어를 찾지 못했습니다. 기록을 백업해 주세요.</div>';const training=t.phase==='learn',writing=['trace','writing'].includes(t.skill);let body='';
 ...
 if(t.skill==='study')body=`${heading('새로운 단어를 만나요.','뜻을 확인한 뒤 소리로도 기억해요.')}<section class="flashcard intro-card">${wordHTML(w,true)}<p class="word-meaning">${esc(w.meaning)}</p><span class="label">${w.level}</span></section>`;
 if(t.skill==='audio'||t.skill==='listening')body=`${heading(training?'소리까지 함께 기억해요.':'듣고, 알맞은 단어를 골라요.',training?'재생이 끝나면 한자를 직접 써 볼 거예요.':'소리를 듣기 전에는 정답을 제출할 수 없어요.')}<section class="listening-stage ${training?'training-audio':''}">${training?wordHTML(w,true):''}<div class="audio-controls"><button class="audio-main" data-action="listen" aria-label="${s.heard?'다시 듣기':'단어 듣기'}">${icon('sound')}<span class="audio-button-label">${s.heard?'다시 듣기':'듣기'}</span>${wave()}</button><button class="slow-button" data-action="slow" aria-label="천천히 듣기">0.7×</button></div><p class="audio-status" id="audio-status">${s.heard?'1회 자동재생 완료 · 더 듣고 싶으면 다시 듣기를 눌러 주세요.':'잠시 후 자동으로 1회 재생합니다.'}</p>${training?`<p class="word-meaning">${esc(w.meaning)}</p>`:''}</section>${training?'':options(t,w)}`;
...
function settings(){openModal(`${modalHead('내 속도에 맞춰요')}<div class="setting-row"><div><b>히라가나 표시</b><p>단어의 읽기를 켜거나 끄기</p></div><input type="checkbox" data-setting="furigana" aria-label="히라가나 표시" ${state.settings.furigana?'checked':''}></div><div class="setting-row"><div><b>화면 애니메이션</b><p>기기의 동작 줄이기도 반영해요</p></div><input type="checkbox" data-setting="motion" aria-label="화면 애니메이션" ${state.settings.motion?'checked':''}></div><div class="setting-row"><div><b>일본어 재생 속도</b></div><select data-setting="rate" aria-label="재생 속도">${[.7,.85,1].map(n=>`<option value="${n}" ${n===state.settings.rate?'selected':''}>${n}×</option>`).join('')}</select></div><div class="setting-row"><div><b>펜 굵기</b></div><select data-setting="penWidth" aria-label="펜 굵기">${[3,4,6].map(n=>`<option value="${n}" ${n===state.settings.penWidth?'selected':''}>${n}px</option>`).join('')}</select></div><div class="setting-row"><div><b>하루 목표</b></div><select data-setting="goal" aria-label="하루 목표">${[5,10,20,30].map(n=>`<option value="${n}" ${n===state.settings.goal?'selected':''}>${n}문제</option>`).join('')}</select></div><div class="modal-actions">${btn('export','기록 백업','soft')}${btn('import','백업 가져오기','soft')}</div><input id="backup-file" type="file" accept="application/json,.json" class="sr-only"><div class="spaced">${btn('packs',icon('download')+' N5~N1 전체 단어팩','soft wide')}</div>${isNative()?`<p class="info-note">손글씨 판정과 모든 획 데이터가 APK에 내장돼 있어요. 첫 실행부터 인터넷 없이, 한 획씩 확인하고 제자리로 보정합니다. 외부 모델이나 별도 설치가 필요 없어요.</p>`:`<p class="info-note">앱과 같은 획순·위치·모양 판정을 사용합니다. 이 기능은 정해진 글자를 따라 쓰는 학습 기능이며 자유 필기 OCR은 아닙니다.</p>`}<p class="fine spaced">오프라인 발음: HTS Voice “NIT ATR503 M001” (CC BY 3.0). Nagoya Institute of Technology / Tokyo Institute of Technology. 자세한 고지는 아래 저작권 페이지에서 확인할 수 있어요.</p><div class="legal-links"><a class="quiet-link" href="./privacy.html">개인정보처리방침</a><a class="quiet-link" href="./terms.html">이용약관</a><a class="quiet-link" href="./licenses.html">오픈소스·저작권</a></div><div class="spaced">${btn('reset','학습 기록 초기화','text danger')}</div>`);}
function packsModal(){if(isNative()){openModal(`${modalHead('앱에 모두 담겨 있어요')}<p>N5~N1 단어팩과 획 데이터가 APK 안에 들어 있어요. 다운로드나 손글씨 모델 준비 없이 사용할 수 있습니다.</p><div class="pack-levels">${LEVELS.map(l=>`<div class="memory-row"><b>${l}</b><span>${packInfo(l).installed.toLocaleString()}단어 내장</span></div>`).join('')}</div><p class="fine">일본어 발음 파일도 APK에 모두 포함되어 첫 실행부터 오프라인으로 재생됩니다. 전체 단어팩은 한국어 뜻만 표시하며, 자동 번역 항목은 출시 전 교육 검수가 필요합니다.</p>`);return;}openModal(`${modalHead('N5부터 N1까지')}<p class="fine">OpenJLPT 고정 버전에서 내려받습니다. 전체 단어팩은 한국어 뜻을 표시합니다. 자동 번역 항목은 출시 전 교육 검수를 진행합니다. 다운로드 시 GitHub에 접속 정보가 전달됩니다. 학습 기록·필기는 전송하지 않습니다.</p><div class="pack-levels">${LEVELS.map(l=>{const p=packInfo(l);return `<div class="pack-level"><div><b>${l}</b><p>${p.installed.toLocaleString()}개 설치 / 원본 ${p.available.toLocaleString()}항목</p><small>${p.complete?`전체팩 · 한국어 뜻`:'한국어 시작 단어 60개'}</small></div>${btn('install-level',p.complete?'다시 받기':'전체팩 설치','soft',`data-level="${l}" ${downloading?'disabled':''}`)}</div>`;}).join('')}</div>${btn('install-all','5개 급수 전체팩 받기','primary wide',downloading?'disabled':'')}<p class="fine spaced">공식 전수 목록이 아닌 공개 자료입니다. 중복 표기는 병합하며 설치 후 실제 단어 수를 표시합니다. 다른 뜻의 예문이 섞인 항목은 일괄 제외합니다.</p><p id="pack-status" role="status"></p>`);}
function detail(id){const w=W(id);if(!w)return;openModal(`${modalHead('단어의 기억')}<div class="detail-word">${wordHTML(w,true)}<p class="word-meaning">${esc(w.meaning)}</p>${btn('word-audio',icon('sound')+' 발음 듣기','soft',`data-id="${w.id}"`)}</div><div class="memory-list">${['meaning','writing','listening'].map(k=>{const r=state.memory[keyOf(id,k)];return `<div class="memory-row"><b>${names[k].replace(' 시험','')}</b><span>${r?dueLabel(r.due):'아직 학습하지 않았어요'}</span></div>`;}).join('')}</div><p class="fine spaced">한국어 뜻 · ${w.source==='openjlpt'?'OpenJLPT / EDRDG 기반':'코토바 자체 편집'}<br>자동 번역된 뜻과 급수 분류는 출시 전 교육 검수가 필요합니다.</p>`);}
...
async function play(slow=false,wordId=null,auto=false){const s=state.session,t=wordId?null:current(s),w=W(wordId||t?.wordId);if(!w)return;const nonce=++audioNonce,rate=slow?.7:state.settings.rate;const status=document.querySelector('#audio-status');if(status)status.textContent=auto?'자동으로 1회 듣는 중이에요…':'발음을 재생하고 있어요…';document.querySelector('.audio-main')?.classList.add('playing');
 ...
 if(nonce!==audioNonce)return;if(t&&state.session===s&&current(s)?.id===t.id&&route()==='lesson'){s.heard=true;await save();render();}}
