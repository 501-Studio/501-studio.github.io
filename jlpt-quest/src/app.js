...
function cancelWork(){audioNonce++;gradeNonce++;stopAudio();if(isNative())callNative('stopAudio',{},3000).catch(()=>{});busy=false;}
...
async function play(slow=false,wordId=null){const s=state.session,t=wordId?null:current(s),w=W(wordId||t?.wordId);if(!w)return;const nonce=++audioNonce,rate=slow?.7:state.settings.rate;const status=document.querySelector('#audio-status');if(status)status.textContent='발음을 재생하고 있어요…';document.querySelector('.audio-main')?.classList.add('playing');
 try{if(isNative())await callNative('speak',{text:w.reading||w.word,rate},25000);else await speak(w.reading||w.word,rate);
 if(nonce!==audioNonce)return;if(t&&state.session===s&&current(s)?.id===t.id&&route()==='lesson'){s.heard=true;await save();if(status)status.textContent='재생 완료 · 이제 다음 단계로 갈 수 있어요.';syncButtons();}}
