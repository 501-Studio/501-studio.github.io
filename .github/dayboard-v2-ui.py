"""Dashboard interaction tests using isolated fake data and mocked cloud responses.
No user key, account, schedule or Google event is read or written.
"""
...
KEY='qa-dashboard-not-a-real-workspace-key';STAMP='2026-09-07T08:00:00+09:00';checks=[];errors=[]
def uid():return str(uuid.uuid4())
def task(title,**data):return dict(id=uid(),kind='task',parentId=None,title=title,deadline=None,estimatedMinutes=60,importance=2,urgency=2,category='일반',recurrence='none',progress=0,status='todo',notes='',pinned=False,pinIndex=None,recommendationRank=None,recommendationAt=None,completedAt=None,createdAt=STAMP,**data)
projects=[];tasks=[]
...
blocks=[]
for n,h in enumerate([9,10,11,14,15,16]):
 blocks.append(dict(id=uid(),taskId=tasks[n]['id'],title=tasks[n]['title'],start=f'2026-09-07T{h:02d}:00:00+09:00',end=f'2026-09-07T{h:02d}:45:00+09:00',source='app',locked=False,allDay=False))
...
def setup(ctx):
 ...
 def rpc(route):
  data=route.request.post_data_json;method=route.request.url.split('/')[-1]
...
with sync_playwright() as p:
 ...
 try:
  ...
  page.locator('[data-action=refresh]').first.click();expect(quick.locator('[name=title]')).to_have_value('옵션 업무 추가');expect(quick.locator('[name=estimatedMinutes]')).to_have_value('75');quick.evaluate('(f)=>f.requestSubmit()');expect(quick.locator('[name=title]')).to_have_value('')
  ...
  page.set_viewport_size({'width':1440,'height':1000});page.locator('[data-action=refresh]').first.click();page.locator('.dash-order-menu summary').click();page.locator('[data-action=gpt-order]').click();expect(page.locator('#chat-prompt')).to_contain_text('recommendationRank');expect(page.locator('#chat-prompt')).to_contain_text(snapshot['workspaceId']);assert KEY not in page.locator('#chat-prompt').input_value();page.locator('#modal [data-action=close]').click();ok('GPT request carries pin contract and current workspace, no private key')
  quick.locator('[name=title]').fill('<img src=x onerror=alert(1)>');quick.evaluate('(f)=>f.requestSubmit()');page.locator('[data-action=toggle-tasks]').click();assert page.locator('.dash-task img').count()==0;ok('Untrusted task markup is escaped')
